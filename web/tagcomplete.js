        ...
        mount(el) {
            el.innerHTML = `
                <div style="max-width:680px">
                  <h2 style="font-family:var(--serif);font-weight:400;margin:0 0 8px">Tag Usage</h2>
                  <p class="hint">Tags you've selected from the autocomplete popup, ranked by frequency.
                    Click the reset button to clear a count.</p>
                  <div id="tac-usage-table" style="margin-top:12px">loading\u2026</div>
                </div>`;
            const container = el.querySelector("#tac-usage-table");
            getAllUseCounts().then(counts => {
                if (!counts || counts.length === 0) {
                    container.innerHTML = '<p class="sub">No tag usage recorded yet. Use the autocomplete popup to start tracking.</p>';
                    return;
                }
                let table = document.createElement("table");
                table.style.cssText = "width:100%;border-collapse:collapse;font-size:13px;background:var(--surface);border:1px solid var(--line);border-radius:var(--r);overflow:hidden";
                let thead = document.createElement("thead");
                thead.innerHTML = `
                    <tr style="text-align:left;border-bottom:1px solid var(--line-hi)">
                        <th style="padding:8px 12px;font-size:11px;font-weight:600;letter-spacing:0.4px;text-transform:uppercase;color:var(--txt-2)">Name</th>
                        <th style="padding:8px 12px;font-size:11px;font-weight:600;letter-spacing:0.4px;text-transform:uppercase;color:var(--txt-2)">Type</th>
                        <th style="padding:8px 12px;font-size:11px;font-weight:600;letter-spacing:0.4px;text-transform:uppercase;color:var(--txt-2)">Count(+)</th>
                        <th style="padding:8px 12px;font-size:11px;font-weight:600;letter-spacing:0.4px;text-transform:uppercase;color:var(--txt-2)">Count(-)</th>
                        <th style="padding:8px 12px;font-size:11px;font-weight:600;letter-spacing:0.4px;text-transform:uppercase;color:var(--txt-2)">Last used</th>
                ...
                counts.forEach(t => {
                    ...
                    tr.style.cssText = "border-bottom:1px solid var(--line);transition:all 120ms ease";
                    tr.onmouseenter = () => { tr.style.background = "var(--surface-2)"; };
                    tr.onmouseleave = () => { tr.style.background = ""; };
                    const typeNames = Object.keys(ResultType);
                    const typeName = typeNames[t.type - 1] || "?";
                    const values = [
                        { v: t.name, style: "color:var(--txt)" },
                        { v: typeName, style: "color:var(--txt-3);font-family:var(--mono);font-size:11px" },
                        { v: t.count, style: "color:var(--accent);font-family:var(--mono);font-size:12px" },
                        { v: t.negCount, style: "color:var(--danger);font-family:var(--mono);font-size:12px" },
                        { v: t.lastUseDate || "", style: "color:var(--txt-3);font-family:var(--mono);font-size:11px" },
                    ];
                    values.forEach(({ v, style }) => {
                        let td = document.createElement("td");
                        td.textContent = v;
                        td.style.cssText = `padding:7px 12px;${style}`;
                        tr.appendChild(td);
                    });
                    let delTd = document.createElement("td");
                    delTd.style.padding = "7px 12px";
                    let btn = document.createElement("button");
                    btn.className = "btn small";
                    btn.textContent = "Reset";
                    btn.title = "Reset count";
                    btn.addEventListener("click", async () => {
                        await resetUseCount(t.name, t.type, true, true);
                        tr.remove();
                    });
